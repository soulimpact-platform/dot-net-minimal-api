...
```text
GET /api/products/search?name={書籍名}&category={カテゴリ}&author={著者名}&minPrice={最小価格}&maxPrice={最大価格}&sortBy={並び替え項目}&sortOrder={並び順}&page={ページ番号}&pageSize={表示件数}
```
...
```text
GET /api/products/export-csv?name={書籍名}&category={カテゴリ}&author={著者名}&minPrice={最小価格}&maxPrice={最大価格}&sortBy={並び替え項目}&sortOrder={並び順}
```
